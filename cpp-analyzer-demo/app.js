...
(function() {
    ...
    // DOM refs
    var $in        = document.getElementById('code-input');
    var $hlIn      = document.getElementById('highlight-input');
    var $hlOut     = document.getElementById('highlight-output');
    var $lnIn      = document.getElementById('line-numbers-input');
    var $lnOut     = document.getElementById('line-numbers-output');
    var $sel       = document.getElementById('example-select');
    var $btnRun    = document.getElementById('btn-analyze');
    ...
    var $scoreCir  = document.getElementById('score-circle');
    var $issues    = document.getElementById('issues-container');
    var $badge     = document.getElementById('refactor-badge');
    var $phOut     = document.getElementById('placeholder-output');

    var lastRefactored = '';

    // ── Line numbers ──
    function updateLineNums(el, count) {
        var h = '';
        for (var i = 1; i <= Math.max(count, 1); i++) h += i + '\n';
        el.textContent = h;
    }

    // ── Highlight sync (both panels use renderOutput for safe + colored display) ──
    function syncHighlight(textarea, hlEl, lnEl) {
        var code = textarea.value;
        hlEl.innerHTML = renderOutput(code) + '\n';
        updateLineNums(lnEl, code.split('\n').length);
    }

    function syncScroll(textarea, hlEl, lnEl) {
    ...
    $in.addEventListener('input', function() {
        syncHighlight($in, $hlIn, $lnIn);
    });
    $in.addEventListener('scroll', function() {
        syncScroll($in, $hlIn, $lnIn);
    });
    ...
    // ── Output highlight layer scroll sync ──
    $hlOut.addEventListener('scroll', function() {
        $lnOut.scrollTop = $hlOut.scrollTop;
    });
    ...
    function runAnalysis() {
        ...
        setTimeout(function() {
            try {
                ...
                var refactored = refactorCode(code);

                lastRefactored = refactored;

                // Show refactored code in output highlight layer（使用安全渲染函数）
                $hlOut.innerHTML = renderOutput(refactored) + '\n';
                updateLineNums($lnOut, refactored.split('\n').length);
                $hlOut.scrollTop = 0;
                $lnOut.scrollTop = 0;

                // Hide placeholder, show action buttons
                $phOut.style.display = 'none';
                $btnCopy.style.display = '';
                $btnDl.style.display = '';
                $badge.style.display = '';
                $badge.textContent = result.issues.length + ' 项已优化';

                // Build stats bar
                var crit = result.counts.critical, warn = result.counts.warning, info = result.counts.info;
                $stats.innerHTML =
                    '<div class="stat-card critical"><div><div class="stat-num">' + crit +
                    '</div><div class="stat-lbl">严重问题</div></div></div>' +
                    '<div class="stat-card warning"><div><div class="stat-num">' + warn +
                    '</div><div class="stat-lbl">警告</div></div></div>' +
                    '<div class="stat-card info"><div><div class="stat-num">' + info +
                    '</div><div class="stat-lbl">建议优化</div></div></div>' +
                    '<div class="stat-card fixed"><div><div class="stat-num">' + result.issues.length +
                    '</div><div class="stat-lbl">检出总数</div></div></div>';

                // Score ring animation
                var circ = 2 * Math.PI * 52;
                var offset = circ * (1 - result.score / 100);
                $scoreCir.style.strokeDasharray = circ;
                $scoreCir.style.strokeDashoffset = circ;
                $scoreCir.classList.remove('warn', 'bad');
                void $scoreCir.offsetWidth;
                $scoreCir.style.strokeDashoffset = offset;
                if (result.score < 50) $scoreCir.classList.add('bad');
                else if (result.score < 75) $scoreCir.classList.add('warn');
                $scoreVal.textContent = result.score;

                // Issues list
                var html = '';
                var sorted = result.issues.slice().sort(function(a, b) {
                    var sv = { critical: 0, warning: 1, info: 2 };
                    return (sv[a.severity]||2) - (sv[b.severity]||2);
                });
                for (var ii = 0; ii < sorted.length; ii++) {
                    var it = sorted[ii];
                    var sevLabel = it.severity === 'critical' ? '严重' :
                                   it.severity === 'warning' ? '警告' : '建议';
                    html += '<div class="issue-card ' + it.severity + '">' +
                        '<div class="issue-head">' +
                        '<span class="issue-line">Line ' + it.line + '</span>' +
                        '<span class="issue-sev">' + sevLabel + '</span></div>' +
                        '<div class="issue-desc">' + it.desc + '</div>' +
                        '<div class="issue-suggest">' + it.suggest + '</div></div>';
                }
                if (sorted.length === 0) {
                    html = '<div class="issue-card info" style="text-align:center">' +
                        '<div class="issue-desc" style="color:var(--green)">代码符合 C++17 规范，未发现问题</div></div>';
                }
    ...
    $btnCopy.addEventListener('click', function() {
        if (!lastRefactored) return;
        function restoreBtn() {
            setTimeout(function() {
                $btnCopy.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg> 复制';
            }, 1500);
        }
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(lastRefactored).then(function() {
                $btnCopy.textContent = '已复制!';
                restoreBtn();
            });
        } else {
            var ta = document.createElement('textarea');
            ta.value = lastRefactored;
    ...
    $btnDl.addEventListener('click', function() {
        if (!lastRefactored) return;
        var blob = new Blob([lastRefactored], { type: 'text/x-c++src' });
        var url = URL.createObjectURL(blob);
        var a = document.createElement('a');
        a.href = url; a.download = 'modernized.cpp';
    ...
    // ── Init ──
    $in.value = EXAMPLES['legacy-malloc'];
    syncHighlight($in, $hlIn, $lnIn);
    $in.focus();
